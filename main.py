...
# --- Загружаем секреты из окружения (на BotHost задаются в настройках, локально — из .env) ---
load_dotenv()
BOT_TOKEN = os.getenv("BOT_TOKEN")
SHEET_ID = os.getenv("SHEET_ID")

# --- Настройка доступа к Google Sheets ---
# ВАЖНО: ключ НЕ хранится в репозитории (репозиторий публичный, иначе Google отзовёт ключ).
# Ключ берём из переменной окружения GOOGLE_CREDENTIALS_B64 (это base64 от JSON-файла ключа).
SCOPES = ['https://www.googleapis.com/auth/spreadsheets', 'https://www.googleapis.com/auth/drive']

_creds_b64 = os.getenv("GOOGLE_CREDENTIALS_B64")
if _creds_b64:
    # Рабочий вариант для хостинга: ключ из переменной окружения
    _creds_info = json.loads(base64.b64decode(_creds_b64))
    creds = Credentials.from_service_account_info(_creds_info, scopes=SCOPES)
else:
    # Запасной вариант для локального запуска: файл рядом с кодом
    # (этот файл НЕ должен попадать в публичный репозиторий — добавьте его в .gitignore)
    creds = Credentials.from_service_account_file(
        "gsmbotproject-9550331e9d20.json", scopes=SCOPES
    )
...
async def process_comment(message: types.Message, state: FSMContext):
    ...
    try:
        # Ищем ПЕРВУЮ пустую строку сверху по столбцу A (Дата). Строка 1 — заголовок.
        # Так случайные значения внизу столбца не сбивают нумерацию.
        dates = worksheet.col_values(1)
        target_row = None
        for i in range(1, len(dates)):  # i=1 соответствует строке 2
            if not str(dates[i]).strip():
                target_row = i + 1
                break
        if target_row is None:
            target_row = max(len(dates) + 1, 2)

        # Если в листе не хватает строк — добавляем, чтобы не упереться в границу сетки.
        if target_row > worksheet.row_count:
            worksheet.add_rows(target_row - worksheet.row_count + 50)
